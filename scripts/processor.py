...
class TextProcessor:
    ...
    def process(self, texts, ratings=None):
        ...
        # If ratings not provided, assume neutral
        if ratings is None:
            ratings = [3] * len(texts)
        ...
        for i, text in enumerate(texts):
            ...
            # Apply rating + text fusion
            final_score, final_label = analyze_sentiment(
                text_sentiment,
                rating
            )
            ...
            cluster_data[cluster_id].append(final_score)
        ...
        summary_lines.append(
            f"Overall Sentiment Score: {overall_sentiment:.3f}\n"
        )
        ...
        for cluster_id, values in cluster_data.items():
            ...
            summary_lines.append(
                f"- Volume: {count} reviews ({percentage:.1f}%)"
            )
